...
keyboard = [
  ...
  [
    Button.url("المطور", "https://t.me/X_77_P")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("**- تم حـذف الحسـاب .. بنجـاح \n- شكـراً لـ استخدامـك البوت**", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      try:
        ...
        await event.respond("ارسل المشكله الى لحلها- @X_77_Q\n**LOGS**\n" + str(e))
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("المالك", "https://t.me/X_77_P")
        ]
    ]
